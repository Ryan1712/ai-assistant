...
**Quyền (theo service layer, nhất quán mô hình hiện có):** cả `delete_task` lẫn `delete_project` đều **chỉ CEO** (`require_ceo`) — khớp nguyên trạng `create_task`/`update_task`/`update_project` đều đang CEO-only ("CEO dựng thế giới").
