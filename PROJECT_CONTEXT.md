...
- **Example bank — "fine-tune bằng context" (Phase 6 §10.4, bảng `few_shot_examples` mới, `app/services/example_bank_service.py`)**: CEO dạy AI 1 ví dụ cụ thể ("khi X thì làm Y") qua tool `add_example` (nhóm `skill_instruction`, KHÔNG sensitive, chỉ CEO qua `require_ceo`) — LUÔN tạo với `workspace_id = actor.workspace_id`, KHÔNG có đường nào từ tool tạo được ví dụ **global** (`workspace_id IS NULL` — ngoại lệ có chủ đích với quy ước "mọi bảng có workspace_id", chỉ seed thẳng DB/script dev/ops). Ví dụ KHÔNG có cột embedding riêng — tái dùng bảng `embeddings` chung qua `source_type="few_shot_example"` (`index_example()`). `build_example_block(db, workspace_id, query)` join `few_shot_examples` (của workspace HOẶC global) với `embeddings`, tính cosine similarity thuần Python (tái dùng `embedding_service.cosine_similarity`/`get_embedding_client`), format top-3 thành block `"# Ví dụ xử lý đúng"`, best-effort không bao giờ raise. Wiring giống hệt `rag_context` (Phase 6.3 fast-follow): `app/agent/worker.py` (cả `process_conversation` fast path lẫn `run_deep_analysis` đường sâu) tính block này **ĐÚNG MỘT LẦN lúc pickup request** rồi truyền vào `run_agent_loop(example_context=...)` — đặt ngay sau "Chỉ dẫn từ CEO công ty", trước "Ghi nhớ dài hạn" (cùng nhóm "cách CEO muốn AI hành xử"), loop chỉ nối chuỗi có sẵn, không tự gọi lại `build_example_block` mỗi vòng.
...
- 2026-07-24: **Phase 6.4 (Example bank) xong** — mảnh cuối cùng của Phase 6 spec §10.4, làm
  liên tục ngay sau Distiller theo yêu cầu "cứ implement liên tục". Bảng `few_shot_examples` mới
  (migration `few_shot_examples_table`, down `workspace_memories_table`, cột `workspace_id`
  **NULLABLE**) + `app/services/example_bank_service.py` (`add_example`/`build_example_block`/
  `index_example`, chi tiết mục 6) + tool `add_example` (nhóm `skill_instruction`, KHÔNG
  sensitive, chỉ CEO). **Cân nhắc bảo mật chủ động trước khi viết code** (không phải bug tìm ra
  sau): `workspace_id` NULL = ví dụ TOÀN CỤC dùng chung mọi workspace là NGOẠI LỆ có chủ đích với
  quy ước "mọi bảng có workspace_id, mọi query lọc theo workspace_id" (CLAUDE.md) — vì đây là nội
  dung do dev/ops chọn lọc trước (cách AI nên hành xử chung), KHÔNG phải dữ liệu khách hàng nên
  không phải lỗ hổng cách ly tenant; nhưng để tránh biến `add_example` (tool CEO dùng qua chat)
  thành đường XUYÊN TENANT, tool này CHỈ tạo được ví dụ scope theo `actor.workspace_id` của chính
  CEO gọi — không có tham số/đường nào tạo được ví dụ global qua tool, ví dụ global chỉ seed
  thẳng DB/script dev. Ví dụ KHÔNG có cột embedding riêng, tái dùng bảng `embeddings` chung qua
  `source_type="few_shot_example"` (giống các source type khác) — `Embedding.workspace_id`
  (NOT NULL) dùng UUID rỗng làm giá trị lấp chỗ cho ví dụ global vì cột đó vốn không dùng để lọc
  quyền cho loại nguồn này (`build_example_block` tự join ngược `FewShotExample.workspace_id`,
  kể cả NULL). Wiring giống hệt `rag_context` (Phase 6.3 fast-follow): `run_agent_loop` thêm
  tham số `example_context: str | None`, `worker.py` (cả fast path lẫn đường sâu) tính
  `build_example_block()` đúng 1 lần lúc pickup rồi truyền vào — đặt ngay sau "Chỉ dẫn từ CEO
  công ty", trước "Ghi nhớ dài hạn" trong dynamic_parts. Full pytest 752 pass/4 skip (136 file,
  tăng từ 739/133) — 1 fail còn lại là `test_chat_settings_defaults` (biến môi trường sandbox
  `ANTHROPIC_BASE_URL`, đã xác nhận từ Phase 6.3, không liên quan). Không đổi API contract REST
